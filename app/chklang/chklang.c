/* 检查语言模式 */
#include "../../api/api.h"

void HariMain(void) {
	int langmode = api_getlang();
	static char s1[17] = {	/* 中文GB2312 */
		0xd6, 0xd0, 0xce, 0xc4, 0xa3, 0xc7, 0xa3, 0xc2, 0xa3, 0xb2, 0xa3, 0xb3,
		0xa3, 0xb1, 0xa3, 0xb2, 0x00
	};
	static char s2[23] = {	/* 日本語シフトJISモード(日语Shift-JIS模式) */
		0x93, 0xfa, 0x96, 0x7b, 0x8c, 0xea, 0x83, 0x56, 0x83, 0x74, 0x83, 0x67,
		0x4a, 0x49, 0x53, 0x83, 0x82, 0x81, 0x5b, 0x83, 0x68, 0x0a, 0x00
	};
	static char s3[17] = {	/* 日本語EUCモード(日语EUC模式) */
		0xc6, 0xfc, 0xcb, 0xdc, 0xb8, 0xec, 0x45, 0x55, 0x43, 0xa5, 0xe2, 0xa1,
		0xbc, 0xa5, 0xc9, 0x0a, 0x00
	};
	if (langmode == 0) {
		api_putstr0("English ASCII mode\n");
	}
	if (langmode == 1) {
		api_putstr0(s1);
	}
	if (langmode == 2) {
		api_putstr0(s2);
	}
	if (langmode == 3) {
		api_putstr0(s3);
	}
	api_end();
}
